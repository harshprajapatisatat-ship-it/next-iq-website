...
import { FileText, Circle, Layers, Layout, Code2 } from 'lucide-react';
...
export interface CarouselProps {
  ...
  loop?: boolean;
  round?: boolean;
  /* When true (round mode only), the pagination dots render inside each
     item's own centered flex column (below the description) instead of
     as a separately absolutely-positioned overlay. Default false keeps
     every existing usage byte-identical. */
  dotsInline?: boolean;
}
...
const DEFAULT_ITEMS: CarouselItem[] = [
  ...
  },
  {
    ...
    id: 5,
    icon: <Code2 className="h-[16px] w-[16px] text-white" />,
  },
];
...
function CarouselItemView({
  ...
  dotsInline, totalItems, activeDotIndex, onDotClick,
}: CarouselItemProps) {
      ...
      >
        {itemsForRender.map((item, index) => (
          <CarouselItemView
            ...
            onDotClick={(idx) => setPosition(loop ? idx + 1 : idx)}
          />
        ))}
      </motion.div>
      {!dotsInline && (
