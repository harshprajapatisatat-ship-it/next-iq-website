...
// Font: variable-axis Roboto Flex, loaded via Google Fonts CSS.
...
interface TextPressureProps {
  ...
  className?: string;
  minFontSize?: number;
  /** Floor/ceiling for the 'wght' axis (rest → peak-at-cursor). Defaults match the original effect (thin at rest). */
  weightRange?: [number, number];
  /** Floor/ceiling for the 'wdth' axis (rest → peak-at-cursor). */
...
export default function TextPressure({
    ...
    const animate = () => {
      ...
      if (titleRef.current) {
        ...
        spansRef.current.forEach((span) => {
          ...
          const wght = weight ? Math.floor(getAttr(d, maxDist, weightRange[0], weightRange[1])) : 400;
  ...
  }, [width, weight, italic, alpha, weightRange, widthRange]);
